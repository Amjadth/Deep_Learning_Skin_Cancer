# ====================================================================================
# WEEK 6 — MAXIMUM PERFORMANCE TRAINING (DENORMALIZED DATA + ALL FIXES)
# ====================================================================================
#
# Purpose:
#   Train and evaluate the baseline CNN on pre-denormalized image data using a
#   GPU-optimized, memory-safe pipeline designed to fully utilize an A40-style GPU
#   without causing RAM/VRAM OOM crashes.
#
# Highlights:
#   • Uses pre-denormalized datasets:
...
#       - Values strictly validated to be in [0, 1]
#
#   • Memory & GPU optimization:
#       - Batch size = 128 (↑ from 64) for better GPU utilization
#       - Memmap-based loading (no full array in RAM)
#       - tf.data generator pipeline with prefetch(buffer_size=2) to save 8–10GB RAM
#       - Optional psutil + nvidia-smi monitoring of CPU/GPU memory
#       - Linux page cache clearing only under high memory pressure
#
#   • Training stability:
#       - Custom WarmupExponentialDecay LR schedule:
#           warmup → exponential decay → constant tail
#       - Adam optimizer with gradient clipping (clipnorm=1.0, clipvalue=0.5)
#       - EarlyStopping + ReduceLROnPlateau for robust convergence
#
#   • Safety & diagnostics:
#       - Comprehensive data validation (range, NaN/Inf, class distribution)
#       - Early diagnostics on first batch:
#           forward pass, initial loss sanity check, gradient norms, label range
#       - MemoryOptimizedCallback:
#           GC + TF session clear each epoch, conditional cache drop, epoch stats
#
#   • Reporting & artefacts:
#       - Saves best model checkpoint and final model:
#           models/denormalized_best_model.h5
#           models/denormalized_final_model.h5
#       - CSV training history: results/denormalized_training_history.csv
#       - Confusion matrix heatmap: results/confusion_matrix.png
#       - Per-class precision/recall/F1: results/per_class_metrics.csv
#       - Model architecture text dump: results/model_architecture.txt
#       - Detailed training log: results/training_log.txt
#       - Rich JSON summaries:
#           results/denormalized_results_complete.json  (full metrics)
#           results/denormalized_results.json           (legacy/compact)
#
#   • Performance comparison:
#       - Tracks actual wall-clock time per epoch and total training time
#       - Prints and logs speedup vs original baseline (~3–4× faster for 100 epochs)
#       - Summarizes resource usage (RAM, GPU utilization) and final accuracy
#
# Requirements:
#   • Week 4/5/denormalization pipeline already run to produce the *_denormalized.npy
#   • TensorFlow 2.x, NumPy, Pandas, Seaborn, Matplotlib, scikit-learn, (optional) psutil
#   • Suitable GPU (e.g., NVIDIA A40) with sufficient VRAM and container RAM (~40–50GB)
# ====================================================================================
...
try:
    ...
    baseline_model = keras.models.load_model(str(MODEL_DIR / 'baseline_model.h5'))
    print(f"✓ Model loaded from baseline_model.h5")
except:
    # If not available, create simple CNN
    print(f"ℹ Creating new baseline CNN...")
...
print(f"✓ Model created: {baseline_model.count_params():,} parameters")
...
MODEL_CHECKPOINT = MODEL_DIR / 'denormalized_best_model.h5'
CHECKPOINT = MODEL_DIR / 'denormalized_checkpoint.h5'
...
# 1. Save models
final_model_path = MODEL_DIR / 'denormalized_final_model.h5'
