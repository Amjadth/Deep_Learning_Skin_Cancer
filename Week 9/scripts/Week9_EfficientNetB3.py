"""
WEEK 9: EFFICIENTNETB3 WITH 300×300 INPUT - PROVEN TEMPLATE
Based on Week9_new.py (70.45% accuracy with EfficientNetB0)
Modified for EfficientNetB3 with 300×300 images and 3-level fallback loading
"""
...
class Config:
    ...
    # Data - CHANGED FOR EFFICIENTNETB3 300x300
    IMAGE_SIZE = (300, 300, 3)  # Changed from (224, 224, 3)
    NUM_CLASSES = 8
    
    # Training Parameters - ADJUSTED FOR 300x300
    BATCH_SIZE = 32  # Reduced from 64 due to larger images (300x300 vs 224x224)
    GRADIENT_ACCUMULATION_STEPS = 8  # Increased from 4 to maintain effective batch 256
    EPOCHS_PHASE1 = 50
    EPOCHS_PHASE2 = 25
    ...
    PREFETCH_BUFFER = 4  # Conservative prefetch (4 batches = 128 samples)
    NUM_PARALLEL_CALLS = tf.data.AUTOTUNE  # Auto-tune parallelism (CPU side)
    CACHE_DATASET = False  # NO caching - would OOM with 64GB data!
    INTERLEAVE_CYCLE = 8  # Moderate parallel loading
    
    # Models
    MODEL_CONFIG = {
        'EfficientNetB0': {'input_shape': (224, 224, 3)},
        'EfficientNetB3': {'input_shape': (300, 300, 3)},  # Native 300x300
    ...
    def get_transfer_dir(cls):
        return cls.get_output_dir() / 'transfer_learning_EB3'  # Changed from EB0
...
def configure_gpu():
    ...
    # Disable XLA - causes crashes with mixed_float16 on this TF/CUDA version
    tf.config.optimizer.set_jit(False)
    print("✅ Advanced GPU optimizations enabled (XLA disabled due to compatibility)")
...
# ============================================
# 3-LEVEL FALLBACK NPY LOADING (FROM WEEK9_EFFICIENTNETB3)
# ============================================
...
# ============================================
# FIXED DATA LOADER WITH 3-LEVEL FALLBACK
# ============================================

class OptimizedDataLoader:
    """High-performance data loader with 3-level fallback for 300x300 data"""
    
    def __init__(self, data_dir):
        self.data_dir = Path(data_dir)
        self.class_names = None
        
        # Expected shapes for 300x300 data
    ...
    def create_optimized_dataset(self, X_path, y_path, batch_size, shuffle=True, name="dataset"):
        """
        Create optimized dataset with 3-level fallback loading for 300x300 images
        """
        
        print(f"\n📂 Creating OPTIMIZED {name}...")
        
        # Determine split name for expected shape
        if 'train' in X_path.name:
            split_name = 'train'
        elif 'val' in X_path.name:
            split_name = 'val'
        else:
            split_name = 'test'
        
        # Load with 3-level fallback
        expected_shape = self.EXPECTED_SHAPES[split_name]
        X_mmap = safe_load_npy_direct(X_path, expected_shape=expected_shape)
        
        if X_mmap is None:
            raise RuntimeError(f"Failed to load {X_path} with any method!")
        
        # Load labels
        try:
            y_full = np.load(y_path, allow_pickle=False)
        except:
            y_full = np.load(y_path, allow_pickle=True)
        
        n_samples = X_mmap.shape[0]
        print(f"  Samples: {n_samples:,}")
        print(f"  Image shape: {X_mmap.shape[1:]}")
        ...
        # NO CACHING - prevent OOM with 64GB data in 46GB RAM
        # We rely on memory-mapped loading instead
        
        # Batch
        dataset = dataset.batch(batch_size, drop_remainder=True)
        
        # Repeat for training (infinite dataset) - CRITICAL for multi-epoch training!
        if shuffle:  # Only repeat training set
        ...
        # Cache validation dataset (safe at 8k images, ~8GB for 300x300)
        if name == "Validation":
    ...
    def get_dataset_for_model(self, model_name):
        ...
        # Load 300x300 data files
        X_train_path = self.data_dir / 'X_train_300.npy'
        y_train_path = self.data_dir / 'y_train.npy'
        X_val_path = self.data_dir / 'X_val_300.npy'
        ...
        for path in [X_train_path, y_train_path, X_val_path, y_val_path]:
            ...
            print(f"   ✅ Found: {path.name} ({path.stat().st_size / (1024**3):.2f} GB)")
...
def train_model_optimized(model, model_name, train_ds, val_ds, output_dir):
    ...
    # Calculate steps (CORRECT FOR BATCH SIZE 32)
    steps_per_epoch = 2000  # 64,000 / 32 = 2,000 steps per epoch
    validation_steps = 250   # 8,000 / 32 = 250 validation steps
...
def main():
    print("\n" + "="*70)
    print("🚀 EFFICIENTNETB3 300×300 - PROVEN TEMPLATE")
    print("="*70)
    ...
    models_to_train = [
        'EfficientNetB3'  # Changed from EfficientNetB0
    ]
